...
var InfoRequest = module.exports = function(req, res, next, options) {
  ...
  this.name = "";
  var m;
  if (req.headers['content-disposition'] && m = (req.headers['content-disposition'] || req.headers['x-content-disposition']).match(/name="([^"]+)"/i)) {
    this.name = m[1];
  } else {
    this.name = req.headers['etag'];
  }
  this.id = req.headers['etag'] + this.name;
  this.size = 0;
  this.next = next;
